...
class _BusinessDetailScreenState extends State<BusinessDetailScreen> {
  ...
  Future<void> _loadRemoteMenu() async {
    if (OfflineMode.enabled) {
      if (mounted) setState(() => _remoteMenuLoading = false);
      return;
    }
    if (widget.business.items.isNotEmpty) {
      if (mounted) setState(() => _remoteMenuLoading = false);
      return;
    }
    try {
      final res = await ApiClient()
          .dio
          .get<List<dynamic>>(ApiPaths.businessProducts(widget.business.id));
      final list = res.data ?? const [];
      final items = list.whereType<Map>().map((m) {
        final mm = Map<String, dynamic>.from(m);
        final p = Product(
          id: (mm['id'] ?? '').toString(),
          businessTypeId: widget.business.businessTypeId,
          name: (mm['name'] ?? '').toString(),
          description: '',
          price: (mm['price'] as num?)?.toDouble() ?? 0,
          category: (mm['category'] ?? 'General').toString(),
        ...
        return BusinessItem.fromProduct(p);
      }).toList();
      if (!mounted) return;
      setState(() {
        _displayBiz = Business.fromPublicApi(
          {
            'id': widget.business.id,
            'name': widget.business.name,
            'type': widget.business.businessTypeId,
            'address': widget.business.address,
          },
          items: items,
        );
        _remoteMenuLoading = false;
      });
    } catch (_) {
      if (mounted) setState(() => _remoteMenuLoading = false);
    }
  ...
  /// Local camera roll, owner-picked HTTPS URL (e.g. Unsplash catalog), or generated stock.
  Widget _itemHeroImage(
    BusinessItem item, {
    required double width,
    required double height,
    BoxFit fit = BoxFit.cover,
  }) {
    final fallback = AppAssetImage(
      businessTypeId: biz.businessTypeId,
  ...
  Widget build(BuildContext context) {
    return Consumer<CartProvider>(
      builder: (context, cartProvider, child) {
        // Food mode: [DefaultTabController] length always matches [TabBar] tabs (no manual dispose bugs).
        if (mode == _Mode.food) {
          ...
          if (n < 1) {
            return PopScope(
              ...
              child: Scaffold(
                ...
                body: const Center(child: Text('No categories')),
              ),
            );
          }
          var initialIndex = cats.indexOf(_selectedCategory);
          if (initialIndex < 0 || initialIndex >= n) initialIndex = 0;
          return DefaultTabController(
            key: ValueKey<String>('food_${biz.id}_${cats.join("|")}'),
            length: n,
              ...
              builder: (ctx) {
                return PopScope(
                  ...
                  onPopInvokedWithResult: (didPop, _) {
                    ...
                    final tc = DefaultTabController.maybeOf(ctx);
                    if (tc != null && tc.index != 0) tc.animateTo(0);
                  },
                  child: Scaffold(
                    backgroundColor: AppColors.backgroundLight,
                    body: Column(
                      ...
                      children: [
                        if (_remoteMenuLoading)
                          LinearProgressIndicator(minHeight: 2, color: color),
                        Expanded(child: _foodBody(cats)),
                      ],
                    ),
  ...
  Widget _foodBody(List<String> categories) {
