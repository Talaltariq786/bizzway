...
import '../rentacar/rentacar_booking_owner_detail_screen.dart';
...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  /// When set with [switchTab] to orders (1), [OrdersScreen] opens this sub-tab (0–4).
  int? _ordersTabRequest;
  ...
  void switchTab(int index) {
    setState(() {
      ...
      if (index != 1) _ordersTabRequest = null;
    });
  }

  void _openOrdersTab(int tabIndex) {
    setState(() {
      _selectedIndex = 1;
      _ordersTabRequest = tabIndex.clamp(0, 4);
    });
  }

  void _clearOrdersTabRequest() {
    if (_ordersTabRequest != null) {
      setState(() => _ordersTabRequest = null);
    }
  }
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      if (!auth.isAuthenticated || !auth.isBusinessOwner) return;
      final biz = context.read<BusinessProvider>();
      await biz.loadBusiness();
      await biz.syncRemoteBusinessWithApi();
      await biz.ensureRemoteBusinessExists();
      if (!mounted) return;
      final typeId = biz.selectedBusiness?.id;
      if (typeId != null && typeId.isNotEmpty) {
        await context.read<ApiCatalogProvider>().loadProductsForBusinessType(
          typeId,
  ...
  Widget build(BuildContext context) {
    ...
    return SlidingDrawerShell(
      ...
      drawer: AppDrawer(
        ...
        onSelectDashboardTab: (i) => switchTab(i),
      ),
      child: Scaffold(
        body: Stack(
          children: [
            AnimatedPageTransition(
              ...
              children: [
                _DashboardHome(
                  ...
                  onOpenOrdersTab: _openOrdersTab,
                ),
                OrdersScreen(
                  initialOrdersTabIndex: _ordersTabRequest,
                  onInitialOrdersTabApplied: _clearOrdersTabRequest,
                ),
...
class _DashboardHome extends StatelessWidget {
  ...
  final void Function(int tabIndex) onOpenOrdersTab;
  ...
  Widget build(BuildContext context) {
    ...
    final rentacarBookings =
        bizId == 'rentacar' ? appts.bookingsForOwnerBusinessType(bizId) : const [];
            ...
            child: Column(
              ...
              children: [
                ...
                DashboardOwnerStatsPanel(
                  ...
                  onOrdersTap: () => onOpenOrdersTab(0),
                  onCustomersTap: () => onSwitchTab(3),
                  onProductsTap: () => onSwitchTab(2),
                  onPendingTap: () => onOpenOrdersTab(1),
                ),
  ...
  Widget _buildRentacarBookingRequests(
    BuildContext context,
    List<dynamic> bookings, // CustomerBooking
    Color themeColor,
  ) {
          ...
          ...bookings.take(4).map(
                (b) => _RentacarOwnerBookingCard(
                  ...
                  onTap: () => Navigator.push(
                    ...
                    MaterialPageRoute(
                      builder: (_) =>
                          RentacarBookingOwnerDetailScreen(booking: b),
                    ),
                  ),
                ),
              ),
...
class _RentacarOwnerBookingCard extends StatelessWidget {
  final dynamic booking; // CustomerBooking
  final Color themeColor;
  final VoidCallback onTap;

  const _RentacarOwnerBookingCard({
    ...
    final statusColor = isPending
        ...
        : status == 'confirmed'
            ...
            : status == 'cancelled'
                ? Colors.red
                : Colors.grey;
    final code = (booking.bookingCode ?? booking.id).toString();
